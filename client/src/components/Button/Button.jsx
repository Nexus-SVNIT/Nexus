...
const Button = ({ isButton = false, href, children, variant = "primary" }) => {
    ...
    return (
        isButton ?
            ...
            :
            <Link className='my-4 px-8 py-3 w-fit rounded-md bg-white/20 border border-white/10 transition-colors  active:scale-95 hover:bg-transparent hover:border-white/100 ' href={href}>
                {children}
            </Link >
